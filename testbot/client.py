...
class Client(object):
    ...
    def register(self):
        try:
            print "client reigister is beginning" + str(self.server_uri+'api/whoami?name='+self.name)
            resp, content = http.request(self.server_uri+'api/whoami?name='+self.name, method="GET")
            
            if resp.status == 200:
                print "client capabilities from register: " + str(self.capabilities)
                ...
                "raised clientexception from client.py reigister"
                raise ClientException("Whoami failed for name "+self.name+'\n'+content)
        except Exception as inst:
            print "!!!!!!!! register in client.py threw an exception"
            print type(inst)
            print inst.args
            print inst
            print "!!!!!!!! end of register exception data"
    ...
    def _do_job(self, job):
        print "CLIENT.PY do_job got called  " + str(job)
